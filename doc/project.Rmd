...
\begin{center}
    \begin{tikzpicture}[
        ...
        \node[state] (S) [below of=R] {Susceptible};
        \node[state] (C) [below of=S] {Colonized};
        \node[state] (D) [right of=S] {Diseased};

        \path[->] (R) edge node {$\alpha$} (S);
        \path[->] (S) edge node {$\beta$} (R);
        \path[->] (S) edge node {$\gamma$} (C);
        \path[->] (C) edge node {$\epsilon$} (D);
        \path[->] (D) edge node {$\theta$} (S);
...
\frac{dR}{dt}=a_r\delta N + \theta S - k_r R - \alpha R \\
\frac{dS}{dt}=a_s\delta N + \alpha R + p \epsilon D -\theta S - kS-\lambda S \\
\frac{dC}{dt}=a_{cn}\delta N + \lambda S - \psi C- kC \\
\frac{dD}{dt}=a_d\delta N + \psi C- p\epsilon D - k_dD \\
