...
V as
$$
\left[\begin{matrix}- k + \psi & 0\\- \psi & e \rho \tau + \epsilon p \left(- \tau + 1\right) + k_{d}\end{matrix}\right]
$$

and the Next Generation Matrix as
$$
\left[\begin{matrix}\frac{\beta_{c} \left(S_{a_0} + S_{f_0}\right)}{- k + \psi} + \frac{\beta_{d} \psi \left(S_{a_0} + S_{f_0}\right)}{\left(- k + \psi\right) \left(e \rho \tau - \epsilon p \left(\tau - 1\right) + k_{d}\right)} & \frac{\beta_{d} \left(S_{a_0} + S_{f_0}\right)}{e \rho \tau - \epsilon p \left(\tau - 1\right) + k_{d}}\\0 & 0\end{matrix}\right]
$$
...
$$
\quad - \frac{\left(S_{a_0} + S_{f_0}\right) \left(- \beta_{c} e \rho \tau + \beta_{c} \epsilon p \tau - \beta_{c} \epsilon p - \beta_{c} k_{d} - \beta_{d} \psi\right)}{\left(k - \psi\right) \left(- e \rho \tau + \epsilon p \tau - \epsilon p - k_{d}\right)}
$$

Where $S_{a_0}$ and $S_{f_0}$ are the two susceptible compartments at baseline.
...
Please fix this Jacob! From, Jacob.
...
0.55 - 1.99 in stochastic simulations.
